...
7. На машине **01-zabbix-server** создайте каталог **`/var/lib/zabbix/.ssh`**
...
```
12. На машине **03-zabbix-db** добавьте в файл `/home/devops/.ssh/authorized_keys` содержимое файла ключа  `/home/devops/.ssh/id_ed25519.pub` с машины **01-zabbix-server**.

13.   На машине **01-zabbix-server** перезапустите Zabbix:
```bash
sudo systemctl restart zabbix-server
```
    
14.  Если ошибка все еще остается, значит существует проблема с SELinux. Попробуйте устранить ее самостоятельно и добиться работы ssh-проверки.
    
15. **Подсказка:** найдите сообщение об ошибке в общем журнале `journalctl -f`
```
...
1.  Перейдите в **Мониторинг-->Последние данные**, в **Узлы сети** укажите `zabbix-db`, в **Теги** укажите `student` **Существует** вместо **Содержит**. И посмотрите ссылку **История** в строке **Check uptime via SSH**
