...
---
### Лабораторная работа
1. Самостоятельно настройте зависимости триггера для проблеммы  **`Linux: Active checks are not available`** и **`Linux: Zabbix agent is not available (or nodata for 30m)`** для хостов **infra**, **winclient** и **zabbix-proxy** от проблемы с отключением сетевого интерфейса (сеть 10.0.10.0/24) на хосте **gw**.
