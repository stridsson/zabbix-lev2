...
**Имя**: "Загрузка CPU слишком высокая у 'zabbix-server' в течение 3 минут". Это будет имя триггера, которое будет отображаться в списках и в других местах.
...
```
Также необходимо на **infra** изменить активные проверки на пассивные. Для этого закомментируйте строки относящиеся к `ActiveServer` и в качестве сервера Zabbix укажите Server=10.0.10.1, поменяйте шаблон с активных на пассивные в web-интерфейсе zabbix сервера и проверьте правила фаерволов на узле - должны быть разрешены входящие соединения для zabbix-агента.
