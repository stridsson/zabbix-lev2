...
- **Name (Имя)** → например: `Средняя загрузка CPU за 30 минут`  
- **Type (Тип)** → `Calculated` (**Вычисляемый**)  
- **Key (Ключ)** → например: `cpu.load.avg30`  
- **Formula (Формула вычислений)** → используйте другие элементы данных этого хоста (обратите внимание возможно именно таких данных как в примере может не существовать, попробуйте отфильтровать элементы по ключу `system.cpu.load`):  

  - `avg(//system.cpu.load,30m)` — среднее значение загрузки CPU за 30 минут.  
  или
...
1. Сохраните и протестируйте.
   
2. Нажмите **Add (Добавить)**.  
3.  Перейдите в **Мониторинг → Latest data**.  
4.  Найдите ваш элемент и убедитесь, что данные обновляются.  
