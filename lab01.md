...
![alt text](img/lab01/lab01b01.png)
...
dnf clean all
```
![alt text](img/lab01/lab01b02.png)
...
![alt text](img/lab01/lab01b03.png)
...
![alt text](img/lab01/lab01b04.png)
...
- Шаг 1 (web-интерфейс можно запустить на машинах **00-ws** и **06-winsrv19**, также выбрать в качестве «языка по умолчанию» (Default language) - русский)
...
14. Подключитесь к ВМ **03-zabbix_db**. Подготовьте сервер Postgresql на zabbix-db (полное имя хоста на вашем стенде может отличаться, но должно включать слово «zabbix-db»)
...
20. **Добавьте новый узел**. Используйте рабочий стол ВМ **00-ws** или проброс портов (`ssh -L 8080:10.0.20.10:80 <your_gw_ip>`).
...
![alt text](img/lab01/lab01b05.png)
...
![alt text](img/lab01/lab01b06.png)
...
>**Интерфейс узла сети**: в данном поле может отображаться IP-адрес физического интерфейса, который вы настроили при создании узла сети на предыдущем шаге (пункт 20)

22. Просмотрите собранные данные
...
23. Если данные не обновляются и график пустой, то обратите внимание на содержимое файла `/var/log/zabbix/zabbix_agentd.log`
```log
...
24. Внесите изменения в файл `/etc/zabbix/zabbix_agentd.conf`. Найдите и исправьте строку начинающуюся на `Server=`
```ini
Server=127.0.0.1,10.0.20.10
```

25. Перезапустите **zabbix-agent**
```bash
...
26. Вернитесь к шагу **22** и дождитесь обновления данных